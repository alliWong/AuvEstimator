...
class GtsamFusionRos():
	...
	def __init__(self):
		...
		# Grab frames
		self.dvl_frame = rospy.get_param('~dvl_frame', 'dvl') # moving frame indicating the IMU placement, z points at IMU z axis etc.
		self.bar_frame = rospy.get_param('~bar_frame', 'bar') # moving frame indicating the IMU placement, z points at IMU z axis etc.
		...
		self.bar_interval = rospy.get_param('~bar_interval', 0) # set zero to use all available GPS messages
		self.dvl_interval = rospy.get_param('~dvl_interval', 0) # set zero to use all available GPS messages
		...
		self.use_gt = rospy.get_param('~use_gt', False) # enable / disable the use of 6DOF pose messages
		# Plot information
		...
		params['sigma_pose_rot'] = rospy.get_param('~sigma_pose_rot', [0.5, 0.5, 0.5]) # rpy [rad] error in 6DOF pose rotation
		# params['sigma_pose_rot'] = rospy.get_param('~sigma_pose_rot', [np.inf, np.inf, 5.0/180.0*np.pi]) # rpy [rad] error in 6DOF pose rotation
		params['dvl_cov'] = rospy.get_param('~dvl_cov', [0.5, 0.5, 0.5]) # error in dvl position
		params['bar_cov'] = rospy.get_param('~bar_cov', [0.5]) # error in bar position
		params['sigma_acc_bias_evol'] = rospy.get_param('~sigma_acc_bias_evol', [1e-5, 1e-5, 1e-5])
		params['sigma_gyr_bias_evol'] = rospy.get_param('~sigma_gyr_bias_evol', [1e-4, 1e-4, 1e-4])
		...
		if self.plot_results:
			...
			self.results = {'IMU': [], 'GT': [], 'BAR': [], 'DVL': [], self.fusion_items: []}
	...
	def run(self):
		...
		if self.bag_file_path: # use rosbag API
			...
			for topic, msg, t in bag.read_messages(topics=[self.gt_topic, self.imu_topic, self.bar_topic, self.dvl_topic]):
				if not init_t:
					init_t = t
					continue
			...
			if self.plot_results:
				...
				plots.plot_gtsam(self.results, self.fusion_items, self.save_dir, self.use_gt, self.use_bar, self.use_dvl)
		else: # subscribe to topics
			self.pose_sub = rospy.Subscriber(
				self.gt_topic, PoseWithCovarianceStamped, self.GtCallback, queue_size=100)
			self.imu_sub = rospy.Subscriber(
				self.imu_topic, Imu, self.ImuCallback, queue_size=1000)
			rospy.spin()
	...
	def GtCallback(self, msg):
		...
		if self.plot_results:
			...
			self.results['GT'].append(
				np.concatenate((np.array([msg.header.stamp.to_sec()]), gt_pos, euler_gt_ori, gt_vel), axis=0))
		if not self.use_gt:
			return
		# if self.use_gt:
		# 	self.__fusion_core.AddAbsolutePoseMeasurement(msg.header.stamp.to_sec(), imu_pos, imu_ori)
